...
function findLastEvenNumber(arr) {
  ...
  return foundLast >= 0 ? foundLast : "Парних чисел не знайдено";
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squareArray = flatArray.map((element) => element ** 2);
  const filteredSquaredArray = squareArray.filter(
...
console.log(capitalizeWords(["apple", "banana", "orange"]));
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((total, current) => total + current.price, 0);
  return totalPrice;
